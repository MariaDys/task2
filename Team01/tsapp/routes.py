...
@app.route('/myprofile')
@login_required #ограничивают доступ только авторизованным пользователям
def myprofile():
...
def search():
    ...
    if tsearch:
        ...
        # masked_tsearch = '*' * len(tsearch) if tsearch else ''
        # app.logger.info('[FUNC] [/search] [Success] User:<%s> Role:<%d> Data:<%s>', current_user.login, rid, masked_tsearch)
